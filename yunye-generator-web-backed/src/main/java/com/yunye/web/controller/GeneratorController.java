...
import com.yunye.maker.meta.MetaValidator;
import com.yunye.web.constant.UserConstant;
import com.yunye.web.exception.ThrowUtils;
import com.yunye.web.manager.CacheManger;
import com.yunye.web.manager.CosManager;
import com.yunye.web.model.dto.generator.*;
import com.yunye.web.service.GeneratorService;
import com.yunye.web.service.UserService;
import com.yunye.web.annotation.AuthCheck;
import com.yunye.web.common.BaseResponse;
import com.yunye.web.common.DeleteRequest;
import com.yunye.web.common.ErrorCode;
import com.yunye.web.common.ResultUtils;
import com.yunye.web.exception.BusinessException;
...
public class GeneratorController {
    ...
    public BaseResponse<Page<GeneratorVO>> listGeneratorVOByPageFast(@RequestBody GeneratorQueryRequest generatorQueryRequest,
        ...
        ValueOperations<String, String> valueOperators = stringRedisTemplate.opsForValue();
        Object cacheValue = cacheManger.get(cacheKey);
        if (cacheValue != null) {
...
//            }, false);
        ...
        //写入缓存
        cacheManger.put(cacheKey, generatorVOPage);
        return ResultUtils.success(generatorVOPage);
    }
    ...
    public void useGenerator(@RequestBody GeneratorUseRequest generatorUseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        File scriptFile = FileUtil.loopFiles(unzipDistDir, 2, null)
                ...
                .filter(file -> {
                    if (file.isFile()) {
                        //注意不是windows下的话使用generator而不是generator.bat
        ...
        //构造命令
        File scriptDirParent = scriptFile.getParentFile();
        //注意如果是mac或者linux系统要用./generator
        ...
        processBuilder.directory(scriptDirParent);
        try {
            Process process = processBuilder.start();

            InputStream inputStream = process.getInputStream();
            BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, "UTF-8")); // 设置编码为 UTF-8
            String line;
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "执行生成器脚本");
        }
        //压缩得到的结果返回给前端
        String generatedPath = scriptDirParent.getAbsolutePath() + "/generated";
    ...
    public void cacheGenerator(@RequestBody GeneratorCacheRequest generatorCacheRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String zipFilPath = getCacheFilePath(id, distPath);
        try {
            cosManager.download(distPath, zipFilPath);
        } catch (Exception e) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "生成器下载失败");
        }
    }
